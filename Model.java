...
class Model {
	...
	void collide (Ball[] b){
		...
		if(Math.sqrt(Math.pow(b[0].radius + b[1].radius,2)) >= dist){ //Collided if distance is lower than the radius of the two balls.
			...
			b[0].x -= instersectD * (0.2 + b[0].x - b[1].x)/dist; //Find overlap distance in x direction(similar triangle), 0.2 disturbs the uniform abit but it is good to have an offset in case it still is stuck
			b[0].y -= instersectD * (0.2  + b[0].y - b[1].y)/dist; //Find overlap distance in y direction(similar triangle), 0.2 disturbs the uniform abit but it is good to have an offset in case it still is stuck
